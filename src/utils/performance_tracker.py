    ...
    def _compute_and_store_metrics(self):
        ...
        metrics = {
            ...
            "latency": {
                ...
                "median_ms": (
                    np.median(self.latency_samples) if self.latency_samples else 0
                ),
                "p95_ms": (
                    np.percentile(self.latency_samples, 95)
                    if self.latency_samples
                    else 0
                ),
                "p99_ms": (
                    np.percentile(self.latency_samples, 99)
                    if self.latency_samples
                    else 0
                ),
            },
    ...
    def get_current_stats(self) -> Dict:
        ...
        with self.lock:
            ...
            stats = {
                ...
                "latency": {
                    "mean_ms": (
                        np.mean(self.latency_samples) if self.latency_samples else 0
                    ),
...
class SimulatedFeedback:
    ...
    def generate_feedback(
        self, prediction: str, tracker: PerformanceTracker, timestamp: str
    ):
        """
